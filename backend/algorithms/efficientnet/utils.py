...
def get_finetune_callbacks(save_path, prefix="model"):
    return [
        EarlyStopping(monitor='val_loss', patience=2, restore_best_weights=True),
        ReduceLROnPlateau(monitor='val_loss', factor=0.3, patience=1, min_lr=1e-7, verbose=1),
        ModelCheckpoint(os.path.join(save_path, f"{prefix}_finetuned.h5"), monitor='val_accuracy', save_best_only=True, verbose=1),
    ]
