...
from algorithms.resnet.model import build_resnet
from algorithms.resnet.utils import (
...
SAVE_DIR = os.path.join(backend_dir, "save_models", "resnet")
...
model, base_model = build_resnet(input_shape=(IMAGE_SIZE, IMAGE_SIZE, CHANNELS), n_classes=N_CLASSES)
model.compile(optimizer=optimizers.Adam(learning_rate=1e-3), loss='categorical_crossentropy', metrics=['accuracy'])

model.fit(train_ds, validation_data=val_ds, epochs=EPOCHS, class_weight=CLASS_WEIGHTS, callbacks=get_callbacks(SAVE_DIR, "ResNet50"), verbose=1)

base_model.trainable = True
model.compile(optimizer=optimizers.Adam(learning_rate=1e-5), loss='categorical_crossentropy', metrics=['accuracy'])

model.fit(train_ds, validation_data=val_ds, epochs=EPOCHS, class_weight=CLASS_WEIGHTS, callbacks=get_finetune_callbacks(SAVE_DIR, "ResNet50"), verbose=1)

test_loss, test_acc = model.evaluate(test_ds, verbose=0)
print(f"\nResNet50 Test Accuracy: {test_acc:.4f}")
model.save(os.path.join(SAVE_DIR, "ResNet50_final.h5"))
print(f"ResNet50 saved to {SAVE_DIR}")
